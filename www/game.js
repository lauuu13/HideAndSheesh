...
function buildSofa(x, z, color, faceZ = 1) {
  ...
  wallBox(0.32, 0.75, 1.1, color, x - 1.44, 0.42, z); // left armrest
  wallBox(0.32, 0.75, 1.1, color, x + 1.44, 0.42, z); // right armrest
...
function buildDiningTable(x, z) {
  wallBox(2.4, 0.75, 1.4, 0x8a6c4a, x, 0.4, z); // table
  box(2.5, 0.04, 1.5, 0xf1ede4, x, 0.78, z, false); // tablecloth accent
}
...
function buildFloor(baseY, level) {
  ...
  if (level === 'GROUND') {
    ...
    texturedPlane(16, 16, 8, baseY + 0.02, -5, TEX.tile, 6, 6);          // kitchen/dining
    texturedPlane(7.6, 4, -11.8, baseY + 0.02, 11, TEX.ceramicTile, 3, 2); // bathroom
    texturedPlane(8, 4, -4, baseY + 0.02, 11, TEX.tile, 3, 2);           // laundry
    texturedPlane(16, 4, 8, baseY + 0.02, 11, TEX.tile, 6, 2);           // storage

    // --- Living room (front-left) ---
    signPlane(2.2, 0.6, -8, 3.6, HOUSE_Z_MIN + 0.05, TEX.signLiving, 0);
    buildSofa(-11, -11, 0x6d597a);
    buildTV(-14.5, -6);
    ...
    buildFridge(14.5, -10.5);
    buildDiningTable(6, -3);
    ...
    wallBox(0.25, wallH, 3, 0xe9e2d0, 12, y, 1.5); // west wall
    wallBox(3.6, wallH, 0.25, 0xe9e2d0, 13.8, y, 0); // south wall
    wallBox(1.2, wallH, 0.25, 0xe9e2d0, 12.6, y, 3); // north wall, west segment
    wallBox(1.2, wallH, 0.25, 0xe9e2d0, 15, y, 3); // north wall, east segment (door gap between)
    box(1.2, 0.4, 0.25, 0x5c3d2e, 13.8, baseY + 2.6, 3, false); // door lintel
    wallBox(0.8, 1.8, 0.5, 0x8a6c4a, 13, baseY + 0.9, 1); // shelving
    wallBox(0.8, 1.8, 0.5, 0x8a6c4a, 15, baseY + 0.9, 1); // more shelving
    ...
    // --- Room-specific flooring (over the base wood floor) ---
    texturedPlane(13, 12, -9.5, baseY + 0.16, -3, TEX.carpet, 5, 4);       // bedroom 1
    texturedPlane(8, 12, 7, baseY + 0.16, -3, TEX.carpet, 4, 4);          // bedroom 2
    texturedPlane(7.6, 4, -11.8, baseY + 0.16, 11, TEX.ceramicTile, 3, 2); // bathroom
    texturedPlane(8, 4, -4, baseY + 0.16, 11, TEX.tile, 3, 2);            // linen room
    texturedPlane(16, 4, 8, baseY + 0.16, 11, TEX.tile, 6, 2);            // storage
    ...
    // --- Walk-in closet (small enclosed room, bedroom 1 corner) ---
    wallBox(0.25, wallH, 3, 0xe9e2d0, -13, y, 1.5); // east wall
    wallBox(2.6, wallH, 0.25, 0xe9e2d0, -14.3, y, 0); // south wall
    wallBox(0.9, wallH, 0.25, 0xe9e2d0, -15.15, y, 3); // north wall, west segment
    wallBox(0.7, wallH, 0.25, 0xe9e2d0, -13.35, y, 3); // north wall, east segment (door gap between)
    box(1, 0.4, 0.25, 0x5c3d2e, -14.25, baseY + 2.6, 3, false); // door lintel
    wallBox(0.5, 1.8, 0.4, 0x8a6c4a, -15, baseY + 0.9, 1); // shelving
    wallBox(0.5, 1.8, 0.4, 0x8a6c4a, -13.5, baseY + 0.9, 1); // more shelving

    // --- Bedroom 2 (front-right) ---
    signPlane(2.2, 0.6, 8, baseY + 3.6, HOUSE_Z_MIN + 0.05, TEX.signBed2, 0);
    buildBed(9, -9, 0xf4a261, baseY);
