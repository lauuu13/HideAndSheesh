...
const AVATAR_SCALE = 0.85; // characters rendered a little smaller than life-size
...
// The stairwell runs along the west side of the house (not the
// center), with two separate flights stacked along Z: floor 0→1,
// then floor 1→2.
const FLOOR_HEIGHT = 8;
const NUM_FLOORS = 3;
const HOUSE_X_MIN = -16, HOUSE_X_MAX = 16, HOUSE_Z_MIN = -13, HOUSE_Z_MAX = 13;
const STAIR_HALL_X_MAX = -11; // stair hall spans HOUSE_X_MIN..STAIR_HALL_X_MAX
const STAIRS_A_Z_MIN = -8, STAIRS_A_Z_MAX = -2; // floor 0 -> 1
const STAIRS_B_Z_MIN = 2, STAIRS_B_Z_MAX = 8;   // floor 1 -> 2

// Resolves how high the "ground" is beneath (x,z), and which floor the
// player is considered to be standing on. Multiple floors share the
// same (x,z) footprint, so floor state persists once off the stairs
// rather than being derivable purely from position. Each stair flight
// only applies when the player's current floor is actually adjacent
// to it — otherwise a floor-2 player standing above the floor-0→1
// stairwell (same X/Z, different floor) would incorrectly get pulled
// down through the building.
function resolveFloorHeight(x, z, currentFloor) {
  const inStairHallX = x >= HOUSE_X_MIN && x <= STAIR_HALL_X_MAX;
  if (inStairHallX && z >= STAIRS_A_Z_MIN && z <= STAIRS_A_Z_MAX && (currentFloor === 0 || currentFloor === 1)) {
    const t = THREE.MathUtils.clamp((z - STAIRS_A_Z_MIN) / (STAIRS_A_Z_MAX - STAIRS_A_Z_MIN), 0, 1);
    return { height: t * FLOOR_HEIGHT, floor: t > 0.5 ? 1 : 0 };
  }
  if (inStairHallX && z >= STAIRS_B_Z_MIN && z <= STAIRS_B_Z_MAX && (currentFloor === 1 || currentFloor === 2)) {
    const t = THREE.MathUtils.clamp((z - STAIRS_B_Z_MIN) / (STAIRS_B_Z_MAX - STAIRS_B_Z_MIN), 0, 1);
    return { height: FLOOR_HEIGHT + t * FLOOR_HEIGHT, floor: t > 0.5 ? 2 : 1 };
  }
...
function syncPlayerList(list) {
  list.forEach((p) => {
    ...
    if (!rp || rp.kind !== desiredKind) {
      ...
      rp = {
        mesh: built.group, parts: built.parts, materials: built.materials, kind: desiredKind,
...
function updateRemoteWalkAnimations(dt) {
  ...
  remotePlayers.forEach((rp) => {
    ...
    if (dist > 0.01) rp.lastMoveAt = now;
    const recentlyMoving = !!rp.lastMoveAt && (now - rp.lastMoveAt) < 300;
    const speedFraction = dt > 0 ? Math.min(1, (dist / dt) / MOVE_SPEED) : 0;
    updateWalkAnimation(rp, dt, speedFraction);
    updateCamouflage(rp, rp.data && rp.data.role, recentlyMoving);
...
let seekerCount = 1;
const seekerCountRow = el('seekerCountRow');
const seeker1Btn = el('seeker1Btn');
const seeker2Btn = el('seeker2Btn');

function setSeekerCount(n) {
  seekerCount = n;
  seeker1Btn.classList.toggle('selected', n === 1);
  seeker2Btn.classList.toggle('selected', n === 2);
}
seeker1Btn.onclick = () => setSeekerCount(1);
seeker2Btn.onclick = () => { if (!seeker2Btn.disabled) setSeekerCount(2); };

function updateStartButtonState(count) {
  startBtn.textContent = `Start Round (Host) — ${count}/${MAX_PLAYERS} players`;
  startBtn.disabled = count < 2;
  seekerCountRow.classList.toggle('hidden', count < 2);
  // 2 officers needs at least 1 hider left over
  seeker2Btn.disabled = count < 3;
  if (seekerCount === 2 && count < 3) setSeekerCount(1);
}
  ...
  return makeCanvasTexture((ctx, s) => {
    ...
    for (let r = 0; r < tiles; r++) {
      for (let c = 0; c < tiles; c++) {
        ctx.fillStyle = (r + c) % 2 === 0 ? 'rgba(255,255,255,0.5)' : 'rgba(179,217,224,0.35)';
        ctx.fillRect(c * (s / tiles), r * (s / tiles), s / tiles, s / tiles);
      }
    }
    ctx.strokeStyle = 'rgba(120,150,155,0.5)';
    ctx.lineWidth = 2;
    for (let i = 0; i <= tiles; i++) {
      const p = i * (s / tiles);
      ctx.beginPath(); ctx.moveTo(p, 0); ctx.lineTo(p, s); ctx.stroke();
...
function makeCarpetTexture() {
  return makeCanvasTexture((ctx, s) => {
    ...
    for (let i = 0; i < 900; i++) {
      ...
      ctx.fillStyle = `rgba(${90 + shade},${78 + shade},${64 + shade},0.5)`;
      ctx.fillRect(Math.random() * s, Math.random() * s, 1.5, 1.5);
    }
  }, 128);
}
...
function initScene() {
  ...
  TEX = {
    ...
    ceramicTile: makeCeramicTileTexture(),
...
// ---- Detailed, multi-part furniture (built from primitives, but
// shaped and layered to actually read as the object they represent) ----
function buildLeg(x, y, z, height, radiusTop = 0.045, radiusBottom = 0.06, color = 0x4a3524) {
  ...
  leg.castShadow = true;
  scene.add(leg);
  world.raycastMeshes.push(leg);
  return leg;
}

function buildSofa(x, z, color, faceZ = 1) {
  wallBox(3.2, 0.55, 1.1, color, x, 0.3, z); // seat base
  wallBox(3.2, 0.7, 0.22, color, x, 0.65, z - faceZ * 0.44); // backrest
  wallBox(0.28, 0.65, 1.1, color, x - 1.44, 0.38, z); // left armrest
  wallBox(0.28, 0.65, 1.1, color, x + 1.44, 0.38, z); // right armrest
  // Rounded front caps on the armrests, instead of a plain box corner
  const capL = new THREE.Mesh(new THREE.CylinderGeometry(0.14, 0.14, 0.65, 12), new THREE.MeshStandardMaterial({ color }));
  capL.rotation.x = Math.PI / 2;
  capL.position.set(x - 1.44, 0.38, z + faceZ * 0.55);
  scene.add(capL); world.raycastMeshes.push(capL);
  const capR = capL.clone();
  capR.position.x = x + 1.44;
  scene.add(capR); world.raycastMeshes.push(capR);
  box(1.2, 0.18, 0.5, 0xf1f3f5, x, 0.62, z + faceZ * 0.2, false); // cushion accent
  buildLeg(x - 1.4, 0.1, z - 0.45, 0.2, 0.04, 0.05, 0x2b2118);
  buildLeg(x + 1.4, 0.1, z - 0.45, 0.2, 0.04, 0.05, 0x2b2118);
  buildLeg(x - 1.4, 0.1, z + 0.45, 0.2, 0.04, 0.05, 0x2b2118);
  buildLeg(x + 1.4, 0.1, z + 0.45, 0.2, 0.04, 0.05, 0x2b2118);
}

function buildTV(x, z, wallFacing = 1) {
  wallBox(1.4, 0.55, 0.4, 0x2b2b2b, x, 0.28, z); // stand
  const screen = box(2, 1.1, 0.08, 0x0a0a0a, x, 1.15, z - wallFacing * 0.15, false);
  screen.material.emissive = new THREE.Color(0x113355);
  screen.material.emissiveIntensity = 0.3;
  box(2.1, 1.2, 0.03, 0x1c1c1c, x, 1.15, z - wallFacing * 0.19, false); // bezel behind screen
}

function buildFridge(x, z) {
  wallBox(1, 2, 1, 0xe9edf0, x, 1, z); // body
  box(0.06, 2, 1.02, 0xc3c9cf, x - 0.15, 1, z, false); // door seam
  box(0.06, 0.3, 0.06, 0x495057, x + 0.35, 1.4, z + 0.51, false); // upper handle
  box(0.06, 0.3, 0.06, 0x495057, x + 0.35, 0.6, z + 0.51, false); // lower handle
}

function buildDiningTable(x, z) {
  wallBox(2.4, 0.08, 1.4, 0x8a6c4a, x, 0.72, z); // thin tabletop, not a solid block
  box(2.5, 0.02, 1.5, 0xf1ede4, x, 0.77, z, false); // tablecloth accent
  buildLeg(x - 1.05, 0.36, z - 0.6, 0.72, 0.05, 0.065);
  buildLeg(x + 1.05, 0.36, z - 0.6, 0.72, 0.05, 0.065);
  buildLeg(x - 1.05, 0.36, z + 0.6, 0.72, 0.05, 0.065);
  buildLeg(x + 1.05, 0.36, z + 0.6, 0.72, 0.05, 0.065);
}
...
function buildChair(x, z, color) {
  wallBox(0.5, 0.08, 0.5, color, x, 0.42, z); // seat
  wallBox(0.5, 0.6, 0.08, color, x, 0.72, z - 0.21); // backrest
  buildLeg(x - 0.2, 0.19, z - 0.2, 0.38, 0.035, 0.045, 0x3a2a1a);
  buildLeg(x + 0.2, 0.19, z - 0.2, 0.38, 0.035, 0.045, 0x3a2a1a);
  buildLeg(x - 0.2, 0.19, z + 0.2, 0.38, 0.035, 0.045, 0x3a2a1a);
  buildLeg(x + 0.2, 0.19, z + 0.2, 0.38, 0.035, 0.045, 0x3a2a1a);
}

function buildBed(x, z, sheetColor, baseY = 0) {
  wallBox(2, 0.5, 3, 0x5c3d2e, x, baseY + 0.25, z); // frame
  box(1.9, 0.25, 2.85, sheetColor, x, baseY + 0.62, z, false); // mattress/sheets
  box(1.9, 0.2, 0.5, 0xffffff, x, baseY + 0.85, z - 1.3, false); // pillow
  for (const [lx, lz] of [[-0.85, -1.35], [0.85, -1.35], [-0.85, 1.35], [0.85, 1.35]]) {
    buildLeg(x + lx, baseY + 0.05, z + lz, 0.1, 0.05, 0.05, 0x2b1d14);
  }
}

function buildDrawerUnit(x, z, w, h, d, color, baseY = 0, drawerCount = 3) {
  wallBox(w, h, d, color, x, baseY + h / 2, z);
  const rowH = h / drawerCount;
  for (let i = 0; i < drawerCount; i++) {
    const ry = baseY + rowH * (i + 0.5);
    box(w * 0.75, rowH * 0.7, 0.03, 0x3a2a1a, x, ry, z + d / 2 + 0.02, false); // drawer face line
    box(w * 0.22, 0.035, 0.035, 0xc9a24a, x, ry, z + d / 2 + 0.04, false); // pull handle
  }
}

function buildShelvingUnit(x, z, w, h, d, color, baseY = 0, shelfCount = 4, doors = false) {
  wallBox(w, h, d, color, x, baseY + h / 2, z);
  const shelfGap = h / shelfCount;
  for (let i = 1; i < shelfCount; i++) {
    box(w * 0.94, 0.025, d * 0.94, 0x2b2118, x, baseY + shelfGap * i, z, false); // shelf line
  }
  if (doors) {
    box(0.02, h * 0.96, d * 0.9, 0x2b2118, x - w * 0.01, baseY + h / 2, z + d / 2 + 0.01, false); // door seam
    for (const dx of [-0.08, 0.08]) {
      box(0.03, 0.14, 0.03, 0xc9a24a, x + dx, baseY + h * 0.5, z + d / 2 + 0.03, false); // handles
    }
  }
}

function buildDesk(x, z, w, d, color, baseY = 0) {
  wallBox(w, 0.06, d, color, x, baseY + 0.74, z); // thin desktop
  buildLeg(x - w / 2 + 0.1, baseY + 0.37, z - d / 2 + 0.1, 0.74, 0.04, 0.05);
  buildLeg(x + w / 2 - 0.1, baseY + 0.37, z - d / 2 + 0.1, 0.74, 0.04, 0.05);
  buildLeg(x - w / 2 + 0.1, baseY + 0.37, z + d / 2 - 0.1, 0.74, 0.04, 0.05);
  buildLeg(x + w / 2 - 0.1, baseY + 0.37, z + d / 2 - 0.1, 0.74, 0.04, 0.05);
  box(w * 0.4, 0.35, d * 0.6, 0x2b2118, x + w * 0.22, baseY + 0.56, z, false); // drawer block under desk
}

function buildWeightBench(x, z, baseY = 0) {
  wallBox(1.6, 0.12, 0.5, 0x2b2b2b, x, baseY + 0.5, z); // padded bench top
  buildLeg(x - 0.65, baseY + 0.25, z - 0.18, 0.5, 0.03, 0.035, 0x111111);
  buildLeg(x + 0.65, baseY + 0.25, z - 0.18, 0.5, 0.03, 0.035, 0x111111);
  buildLeg(x - 0.65, baseY + 0.25, z + 0.18, 0.5, 0.03, 0.035, 0x111111);
  buildLeg(x + 0.65, baseY + 0.25, z + 0.18, 0.5, 0.03, 0.035, 0x111111);
}

function buildWeightRack(x, z, baseY = 0) {
  buildLeg(x - 0.3, baseY + 0.8, z, 1.6, 0.04, 0.04, 0x495057);
  buildLeg(x + 0.3, baseY + 0.8, z, 1.6, 0.04, 0.04, 0x495057);
  for (const ry of [0.4, 0.9, 1.4]) {
    const bar = new THREE.Mesh(new THREE.CylinderGeometry(0.03, 0.03, 0.7, 8), new THREE.MeshStandardMaterial({ color: 0x495057 }));
    bar.rotation.z = Math.PI / 2;
    bar.position.set(x, baseY + ry, z);
    scene.add(bar); world.raycastMeshes.push(bar);
  }
  for (const rx of [-0.42, 0.42]) {
    const plate = new THREE.Mesh(new THREE.CylinderGeometry(0.16, 0.16, 0.05, 16), new THREE.MeshStandardMaterial({ color: 0x1c1c1c }));
    plate.rotation.z = Math.PI / 2;
    plate.position.set(x + rx, baseY + 0.4, z);
    scene.add(plate); world.raycastMeshes.push(plate);
  }
}
...
function buildMap() {
  ...
  buildStaircaseFlight(STAIRS_A_Z_MIN, STAIRS_A_Z_MAX, 0);
  buildStaircaseFlight(STAIRS_B_Z_MIN, STAIRS_B_Z_MAX, FLOOR_HEIGHT);
  buildHallSlabWithGap(0, null, null);                                // ground: solid, no gap
  buildHallSlabWithGap(FLOOR_HEIGHT, STAIRS_A_Z_MIN, STAIRS_A_Z_MAX);  // floor 1: gap for stairs A
  buildHallSlabWithGap(FLOOR_HEIGHT * 2, STAIRS_B_Z_MIN, STAIRS_B_Z_MAX); // floor 2: gap for stairs B

  // Roof cap (purely visual — players can't jump high enough to reach it)
  box(HOUSE_X_MAX - HOUSE_X_MIN + 0.6, 0.3, HOUSE_Z_MAX - HOUSE_Z_MIN + 0.6, 0x5c3d2e, (HOUSE_X_MIN + HOUSE_X_MAX) / 2, FLOOR_HEIGHT * NUM_FLOORS + 0.15, 0, false);
}

// Fills the side stair hall's floor at a given height, minus a Z-range
// gap for whichever stair flight passes through at that level. Only
// the hall ever needs a gap — the main room area is always solid,
// since the stairs live entirely within the hall's footprint.
function buildHallSlabWithGap(atY, gapZMin, gapZMax) {
  const hallW = STAIR_HALL_X_MAX - HOUSE_X_MIN;
  const hallCenterX = (HOUSE_X_MIN + STAIR_HALL_X_MAX) / 2;
  if (gapZMin === null) {
    texturedBox(hallW, 0.3, HOUSE_Z_MAX - HOUSE_Z_MIN, hallCenterX, atY, (HOUSE_Z_MIN + HOUSE_Z_MAX) / 2, { map: TEX.floorWood, repeatU: 2, repeatV: 8 });
    return;
  }
  if (gapZMin > HOUSE_Z_MIN) {
    texturedBox(hallW, 0.3, gapZMin - HOUSE_Z_MIN, hallCenterX, atY, (HOUSE_Z_MIN + gapZMin) / 2, { map: TEX.floorWood, repeatU: 2, repeatV: 3 });
  }
  if (gapZMax < HOUSE_Z_MAX) {
    texturedBox(hallW, 0.3, HOUSE_Z_MAX - gapZMax, hallCenterX, atY, (gapZMax + HOUSE_Z_MAX) / 2, { map: TEX.floorWood, repeatU: 2, repeatV: 3 });
  }
}

// Visual staircase steps for one flight — the walkable ramp itself is
// handled by resolveFloorHeight() elsewhere, this is just its look.
function buildStaircaseFlight(zMin, zMax, floorBaseHeight) {
  const steps = 14;
  const stepDepth = (zMax - zMin) / steps;
  const hallCenterX = (HOUSE_X_MIN + STAIR_HALL_X_MAX) / 2;
  for (let i = 0; i < steps; i++) {
    const t = (i + 0.5) / steps;
    const stepZ = zMin + t * (zMax - zMin);
    const stepY = floorBaseHeight + t * FLOOR_HEIGHT;
    box(4, 0.25, stepDepth + 0.05, 0x8a6c4a, hallCenterX, stepY, stepZ, true);
  }
}

// The wall separating the side stair hall from the main rooms, with
// doorways into the front and back room on that floor.
function buildStairHallWall(floorIndex) {
  const baseY = floorIndex * FLOOR_HEIGHT;
  const y = baseY + FLOOR_HEIGHT / 2;
  wallBox(0.25, FLOOR_HEIGHT, 4, 0xe9e2d0, STAIR_HALL_X_MAX, y, -11);   // north segment
  wallBox(0.25, FLOOR_HEIGHT, 16, 0xe9e2d0, STAIR_HALL_X_MAX, y, 1);    // middle segment
  wallBox(0.25, FLOOR_HEIGHT, 2, 0xe9e2d0, STAIR_HALL_X_MAX, y, 12);    // south segment
  box(2.2, 0.4, 0.25, 0x5c3d2e, STAIR_HALL_X_MAX, baseY + 2.6, -8, false); // lintel, front doorway
  box(2.2, 0.4, 0.25, 0x5c3d2e, STAIR_HALL_X_MAX, baseY + 2.6, 10, false); // lintel, back doorway
}

function buildFloor(floorIndex) {
  const baseY = floorIndex * FLOOR_HEIGHT;
  const wallH = FLOOR_HEIGHT;
  const y = baseY + wallH / 2;

  // Ground floor is one solid plane. Floors 1 and 2's surfaces come
  // entirely from buildFloorSlabWithOpening (called in buildMap), which
  // leaves a hole only over whichever staircase passes through them —
  // building a second, solid plane here would just paper over that hole.
  if (floorIndex === 0) {
    texturedPlane(HOUSE_X_MAX - HOUSE_X_MIN, HOUSE_Z_MAX - HOUSE_Z_MIN, (HOUSE_X_MIN + HOUSE_X_MAX) / 2, baseY + 0.01, 0, TEX.floorWood, 10, 8);
  }

  // Exterior walls
  wallBox(HOUSE_X_MAX - HOUSE_X_MIN + 0.6, wallH, 0.3, 0xe9e2d0, 0, y, HOUSE_Z_MIN);
  wallBox(HOUSE_X_MAX - HOUSE_X_MIN + 0.6, wallH, 0.3, 0xe9e2d0, 0, y, HOUSE_Z_MAX);
  wallBox(0.3, wallH, HOUSE_Z_MAX - HOUSE_Z_MIN + 0.6, 0xe9e2d0, HOUSE_X_MIN, y, 0);
  wallBox(0.3, wallH, HOUSE_Z_MAX - HOUSE_Z_MIN + 0.6, 0xe9e2d0, HOUSE_X_MAX, y, 0);

  // Window panes on the exterior walls (visual only)
  const winY = baseY + wallH * 0.55;
  for (const wx of [-5, 0, 6, 11]) {
  ...
  buildStairHallWall(floorIndex);
  addLampFixture(-4, baseY + wallH - 0.3, -6);
  addLampFixture(9, baseY + wallH - 0.3, -6);
  addLampFixture(9, baseY + wallH - 0.3, 8);

  if (floorIndex === 0) {
    // --- Room-specific flooring (over the base wood floor) ---
    texturedPlane(13.5, 16, -4.25, baseY + 0.02, -5, TEX.floorWood, 5, 6);   // living room (kept as wood, listed for clarity)
    texturedPlane(13.5, 16, 9.25, baseY + 0.02, -5, TEX.tile, 5, 6);         // kitchen/dining
    texturedPlane(13, 10, -4.5, baseY + 0.02, 8, TEX.ceramicTile, 5, 4);     // bathroom
    texturedPlane(13.5, 10, 9.25, baseY + 0.02, 8, TEX.tile, 5, 4);          // laundry/storage

    // --- Living room (front-left) ---
    signPlane(2.2, 0.6, -4.25, 3.6, HOUSE_Z_MIN + 0.05, TEX.signLiving, 0);
    buildSofa(-4, -11, 0x6d597a);
    buildTV(-4, -6);
    wallBox(1.4, 0.5, 0.8, 0x8a6c4a, -2, 0.25, -8); // coffee table
    buildShelvingUnit(1.5, -2, 0.4, 2, 2, 0x5c3d2e, 0, 4);
    wallBox(1, 1, 1, 0x557153, -8, 0.5, -9); // armchair
    wallBox(0.5, 1.6, 0.5, 0x2f6f4f, -8, 0.8, -3); // floor plant

    // --- Kitchen / dining (front-right) ---
    signPlane(2.2, 0.6, 12, 3.6, HOUSE_Z_MIN + 0.05, TEX.signKitchen, 0);
    wallBox(7, 1, 0.8, 0xced4da, 12.3, 0.5, -12.3); // counter
    buildFridge(15.5, -10.5);
    buildDiningTable(9, -3);
    for (const cx of [7.9, 10.1]) for (const cz of [-3.9, -2.1]) {
      buildChair(cx, cz, 0x6d597a);
    }
    wallBox(1, 2.2, 0.6, 0xced4da, 4, 1.1, -6); // pantry cabinet
    wallBox(2, 0.9, 1, 0xced4da, 13, 0.45, -7); // kitchen island
    wallBox(0.4, 0.6, 0.4, 0x495057, 15.5, 0.3, -12.5); // trash bin

    // --- Bathroom (back-left) ---
    wallBox(5, wallH, 0.25, 0xe9e2d0, -6.5, y, 3);   // front wall, west segment
    wallBox(4, wallH, 0.25, 0xe9e2d0, 0, y, 3);      // front wall, east segment (door gap between)
    box(2.1, 0.4, 0.25, 0x5c3d2e, -2.75, baseY + 2.6, 3, false); // door lintel
    wallBox(0.6, 0.8, 0.6, 0xffffff, -6, 0.4, 6); // toilet
    wallBox(0.8, 0.9, 0.5, 0xffffff, -3, 0.45, 6.7); // sink
    wallBox(1.8, 0.7, 3, 0xffffff, -8, 0.35, 9.5); // tub
    wallBox(1.2, 1.8, 0.5, 0x8a6c4a, -6, 0.9, 12); // shelving unit

    // --- Laundry / storage (back-right) ---
    wallBox(1, 1.2, 1, 0xadb5bd, 9, 0.6, 6); // washing machine
    wallBox(1.4, 1.4, 1.4, 0x8a6c4a, 13, 0.7, 6.5); // stacked crates
    wallBox(1.4, 1, 1.4, 0x8a6c4a, 15, 0.5, 9.5); // more crates
    wallBox(1, 1, 1, 0x8a6c4a, 9, 0.5, 11.5); // extra crates
    wallBox(0.6, 1, 0.6, 0xadb5bd, 12, 0.5, 11); // utility bin

    // --- Pantry (small enclosed room, kitchen corner) ---
    wallBox(0.25, wallH, 3, 0xe9e2d0, 4.5, y, -1.5); // west wall
    wallBox(3.2, wallH, 0.25, 0xe9e2d0, 6, y, -3); // north wall
    wallBox(1.1, wallH, 0.25, 0xe9e2d0, 4.9, y, 0); // south wall, west segment
    wallBox(1.1, wallH, 0.25, 0xe9e2d0, 7.1, y, 0); // south wall, east segment (door gap between)
    box(1.2, 0.4, 0.25, 0x5c3d2e, 6, baseY + 2.6, 0, false); // door lintel
    wallBox(0.8, 1.8, 0.5, 0x8a6c4a, 5.5, baseY + 0.9, -1.8); // shelving
    wallBox(0.8, 1.8, 0.5, 0x8a6c4a, 7.5, baseY + 0.9, -1.8); // more shelving
  } else if (floorIndex === 1) {
    // --- Room-specific flooring ---
    texturedPlane(13.5, 16, -4.25, baseY + 0.16, -5, TEX.carpet, 5, 6);      // bedroom 1
    texturedPlane(13.5, 16, 9.25, baseY + 0.16, -5, TEX.carpet, 5, 6);       // bedroom 2
    texturedPlane(13, 10, -4.5, baseY + 0.16, 8, TEX.ceramicTile, 5, 4);     // bathroom
    texturedPlane(13.5, 10, 9.25, baseY + 0.16, 8, TEX.tile, 5, 4);          // linen/storage

    // --- Bedroom 1 (front-left) ---
    signPlane(2.2, 0.6, -4.25, baseY + 3.6, HOUSE_Z_MIN + 0.05, TEX.signBed1, 0);
    buildBed(-4, -9, 0x94a89a, baseY);
    buildDrawerUnit(-2, -5, 1.4, 1, 0.6, 0x8a6c4a, baseY, 3);
    buildDrawerUnit(-7, -6, 1, 0.9, 0.8, 0x8a6c4a, baseY, 2);
    wallBox(0.5, 1.6, 0.5, 0x2f6f4f, -8, baseY + 0.8, -3); // floor plant
    buildShelvingUnit(1.5, -2, 0.6, 2.2, 2, 0x5c3d2e, baseY, 4, true);

    // --- Bedroom 2 (front-right) ---
    signPlane(2.2, 0.6, 12, baseY + 3.6, HOUSE_Z_MIN + 0.05, TEX.signBed2, 0);
    buildBed(12, -9, 0xf4a261, baseY);
    buildDesk(15, -3, 1.6, 0.7, 0x8a6c4a, baseY);
    buildShelvingUnit(15.5, -8, 0.4, 2, 2, 0x5c3d2e, baseY, 4);
    wallBox(1, 0.5, 1.4, 0x557153, 12, baseY + 0.25, -10.5); // bench at foot of bed
    wallBox(0.6, 1.2, 0.6, 0x8a6c4a, 8, baseY + 0.6, -3); // side cabinet

    // --- Home office nook, split off from Bedroom 2 ---
    wallBox(0.25, wallH, 3.6, 0xe9e2d0, 10.5, y, -1.2);
    buildDesk(13.5, 0.5, 1.4, 0.6, 0x8a6c4a, baseY);
    wallBox(0.6, 1, 0.6, 0x557153, 13.5, baseY + 0.5, 2); // office chair
    buildDrawerUnit(15.3, 1, 0.4, 2, 1.4, 0x5c3d2e, baseY, 4);

    // --- Bathroom (back-left) ---
    wallBox(5, wallH, 0.25, 0xe9e2d0, -6.5, y, 3);
    wallBox(4, wallH, 0.25, 0xe9e2d0, 0, y, 3);
    box(2.1, 0.4, 0.25, 0x5c3d2e, -2.75, baseY + 2.6, 3, false);
    wallBox(0.6, 0.8, 0.6, 0xffffff, -6, baseY + 0.4, 6);
    wallBox(0.8, 0.9, 0.5, 0xffffff, -3, baseY + 0.45, 6.7);
    wallBox(1, 1.8, 0.5, 0x8a6c4a, -6, baseY + 0.9, 12); // linen shelf

    // --- Linen room / storage (back-right) ---
    wallBox(2, 2, 0.6, 0x8a6c4a, 11, baseY + 1, 12); // linen closet
    wallBox(1.2, 1.2, 1.2, 0x8a6c4a, 15, baseY + 0.6, 6); // storage boxes
    wallBox(0.8, 0.8, 0.8, 0x8a6c4a, 9, baseY + 0.4, 11); // extra boxes
  } else {
    // --- Floor 3: Game Room + Home Gym + Bathroom + Attic Storage ---
    texturedPlane(13.5, 16, -4.25, baseY + 0.16, -5, TEX.floorWood, 5, 6);   // game room
    texturedPlane(13.5, 16, 9.25, baseY + 0.16, -5, TEX.tile, 5, 6);         // home gym
    texturedPlane(13, 10, -4.5, baseY + 0.16, 8, TEX.ceramicTile, 5, 4);     // bathroom
    texturedPlane(13.5, 10, 9.25, baseY + 0.16, 8, TEX.tile, 5, 4);          // attic storage

    // --- Game room (front-left) ---
    signPlane(2.2, 0.6, -4.25, baseY + 3.6, HOUSE_Z_MIN + 0.05, TEX.signLiving, 0);
    buildSofa(-4, -10, 0x557153);
    buildTV(-4, -5);
    wallBox(1.4, 0.8, 1.4, 0x8a6c4a, -2, baseY + 0.4, -7); // game table
    wallBox(0.4, 2, 2, 0x5c3d2e, 1.5, baseY + 1, -2); // shelving
    wallBox(0.6, 1.2, 0.6, 0x495057, -8, baseY + 0.6, -3); // speaker/console stand

    // --- Home gym (front-right) ---
    signPlane(2.2, 0.6, 12, baseY + 3.6, HOUSE_Z_MIN + 0.05, TEX.signKitchen, 0);
    buildWeightBench(9, -10, baseY);
    buildWeightRack(12, -10, baseY);
    wallBox(1.2, 1.6, 0.5, 0xced4da, 15, baseY + 0.8, -3); // mirror panel
    wallBox(2, 0.1, 1.2, 0x2b2b2b, 9, baseY + 0.06, -4, false); // floor mat
    ...
    // --- Attic storage (back-right) ---
    wallBox(1.4, 1.4, 1.4, 0x8a6c4a, 9, baseY + 0.7, 6); // stacked boxes
    wallBox(1.2, 1.2, 1.2, 0x8a6c4a, 13, baseY + 0.6, 11); // more boxes
    wallBox(0.8, 0.8, 0.8, 0x8a6c4a, 15, baseY + 0.4, 6.5); // small boxes
    wallBox(0.5, 1.6, 0.5, 0x8a6c4a, 11, baseY + 0.8, 11.5); // old lamp/clutter
  }
}
...
function addFace(group, headY) {
  // LEGO-minifig style: bold, flat graphic eyes (printed-look discs,
  // not soft rounded bumps) and a simple thick smile line, mounted on
  // the front of the cylindrical head.
  const eyeGeo = new THREE.CylinderGeometry(0.05, 0.05, 0.025, 12);
  const eyeMat = new THREE.MeshStandardMaterial({ color: 0x1a1a1a });
  const eyeL = new THREE.Mesh(eyeGeo, eyeMat);
  eyeL.rotation.x = Math.PI / 2;
  eyeL.position.set(-0.115, headY + 0.03, 0.27);
  const eyeR = new THREE.Mesh(eyeGeo, eyeMat);
  eyeR.rotation.x = Math.PI / 2;
  eyeR.position.set(0.115, headY + 0.03, 0.27);
  group.add(eyeL, eyeR);

  const mouth = new THREE.Mesh(
    new THREE.TorusGeometry(0.1, 0.02, 6, 10, Math.PI * 0.65),
    new THREE.MeshStandardMaterial({ color: 0x1a1a1a })
  );
  mouth.position.set(0, headY - 0.14, 0.2);
  mouth.rotation.set(-0.15, 0, Math.PI + 0.15);
  group.add(mouth);
}

function addHair(group, headY, color) {
  // Partial sphere "cap" covering the top of the cylindrical head
  const hair = new THREE.Mesh(
    new THREE.SphereGeometry(0.29, 16, 12, 0, Math.PI * 2, 0, Math.PI * 0.55),
    new THREE.MeshStandardMaterial({ color })
  );
  hair.position.y = headY + 0.08;
...
function buildAvatar(kind) {
  ...
  if (kind === 'seeker') {
    ...
    const head = new THREE.Mesh(
      new THREE.CylinderGeometry(0.28, 0.28, 0.42, 20),
    ...
    const materials = [];
    group.traverse((obj) => { if (obj.material) materials.push(obj.material); });
...
// ============================================================
// GHOST MODE — hiders camouflage (near-invisible) when standing
// still, and become partially visible when moving. Seekers are
// always fully visible; this only ever affects hiders.
// ============================================================
const CAMOUFLAGE_STILL_OPACITY = 0.05;
const CAMOUFLAGE_MOVING_OPACITY = 0.4;
function updateCamouflage(avatarEntry, role, moving) {
  if (!avatarEntry || !avatarEntry.materials) return;
  const targetOpacity = role === 'HIDER' ? (moving ? CAMOUFLAGE_MOVING_OPACITY : CAMOUFLAGE_STILL_OPACITY) : 1;
  avatarEntry.materials.forEach((mat) => {
...
function rebuildLocalAvatar() {
  ...
  scene.add(built.group);
  localAvatarEntry = { mesh: built.group, parts: built.parts, materials: built.materials, kind, walkPhase: 0 };
}

function spawnLocalPlayer() {
  // Stair hall, front landing — clear of all colliders, before the
  // first flight of stairs begins
  playerState.x = -13.5 + (Math.random() - 0.5) * 3;
...
function updateThirdPersonCamera() {
  ...
  let camY = playerState.y + 1.6 + CAM_HEIGHT + duckOffset - pitch * 2.0;
  camY = Math.min(camY, floorBaseY + FLOOR_HEIGHT - 0.5); // don't poke through the ceiling
...
function updateLocalPlayer(dt) {
  ...
  if (localAvatarEntry) {
    ...
    localAvatarEntry.mesh.scale.set(AVATAR_SCALE, playerState.ducking ? AVATAR_SCALE * 0.6 : AVATAR_SCALE, AVATAR_SCALE);
    updateWalkAnimation(localAvatarEntry, dt, speedFraction);
    updateCamouflage(localAvatarEntry, myRole, speedFraction > 0.04);
  }
...
function drawMinimap() {
  ...
  if (myAlive) {
    ...
    floor = Math.min(NUM_FLOORS - 1, Math.round(rp.mesh.position.y / FLOOR_HEIGHT));
  } else {
    return;
  }
  ...
  minimapCtx.fillText(floor === 2 ? '3F' : floor === 1 ? '2F' : '1F', 6, 14);
...
startBtn.onclick = () => send({ type: 'START_GAME', seekerCount });
...
browseBtn.onclick = async () => {
  ...
  lobbies.forEach((l) => {
    ...
    row.innerHTML = `<div>
        <div class="lobby-name">${l.name || 'Hide and Sheesh Lobby'}</div>
